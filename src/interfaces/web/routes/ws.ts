  ...
  ws.on('message', async (data) => {
    try {
      ...
      try {
        ...
        let taskConversionResult: { taskId?: string; success: boolean } | undefined;
        try {
          const { convertMessageToTask } = await import('../../../agents/tools/message-to-task-converter.js');
          taskConversionResult = await convertMessageToTask(
            text,
            user.userId,
            'web',
            {
              confidenceThreshold: 0.75,
              autoDispatch: false, // Let user review before dispatching
            }
          );
          if (taskConversionResult.success) {
            logger.info('Auto task conversion succeeded', {
              userId: user.userId,
              taskId: taskConversionResult.taskId,
            });
          }
        } catch (convError) {
          logger.warn('Auto task conversion failed', { error: convError });
        }
        ...
        const responseData = {
          ...
          modelDisplay: (response as any).modelDisplay || response.modelUsed,
          ...
          taskCreated: taskConversionResult?.success ? {
            taskId: taskConversionResult.taskId,
            message: '消息已转换为任务，可在旨意看板查看',
          } : undefined,
