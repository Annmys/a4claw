/**
 * Facebook Account API routes — manage Facebook accounts via cookies.
 */
import { Router, Request, Response } from 'express';
import { FacebookAccountManager } from '../../../actions/browser/facebook-manager.js';
import { parseFacebookCookies, validateFacebookCookies } from '../../../actions/browser/facebook-cookies.js';
import logger from '../../../utils/logger.js';

export function setupFacebookRoutes(): Router {
  const router = Router();
  const mgr = FacebookAccountManager.getInstance();

  /** GET /api/facebook/accounts — list all accounts */
  ...
  /** GET /api/facebook/accounts/:id — get single account (no cookies) */
  ...
  /** POST /api/facebook/accounts — add a new account from cookies */
  router.post('/accounts', (req: Request, res: Response) => {
    try {
      ...
      logger.warn('Failed to add Facebook account', { error: err.message });
      res.status(400).json({ error: err.message });
    }
  });

  /** PUT /api/facebook/accounts/:id/cookies — update cookies for an account */
  ...
  /** DELETE /api/facebook/accounts/:id — delete an account */
  ...
  /** POST /api/facebook/accounts/:id/verify — verify cookies work (headless test) */
  ...
  /** POST /api/facebook/accounts/:id/launch — launch a browser session with cookies */
  ...
  /** POST /api/facebook/parse-preview — preview cookie parsing without saving */
  router.post('/parse-preview', (req: Request, res: Response) => {
    try {
      const { cookies } = req.body ?? {};
      if (!cookies) { res.status(400).json({ error: 'Cookies required' }); return; }

      const parseResult = parseFacebookCookies(cookies);
