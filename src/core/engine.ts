...
If they want to check a running project, use "status" with projectName.`,
      ...
      if (isFileDeliveryRequest(incoming.text) && responseArtifacts.length === 0) {
        try {
          ...
          const fileContent =
            extractExplicitFileContent(incoming.text)
            ?? (typeof response.content === 'string' && response.content.trim() ? response.content.trim() : null)
            ?? incoming.text.trim()
            ?? 'Generated by a4claw';
          ...
          if (typeof response.content === 'string' && isAskingForSavePath(response.content)) {
