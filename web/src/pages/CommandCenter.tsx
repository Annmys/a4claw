...
import { ApprovalGateManager } from '../components/ApprovalGateManager';
import {
  Activity,
  AlertCircle,
  ArrowRight,
...
export default function CommandCenter() {
  ...
  const loadDAG = async () => {
    try {
      const { dag } = await api.get('/command-center/dag');
      setDagData(dag);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col bg-dark-950">
      <div className="px-6 py-5 border-b border-gray-800/70 bg-dark-900">
        ...
        {/* View Tabs */}
        <div className="mt-4 flex items-center gap-2 border-b border-gray-800 pb-2">
          {[
            { id: 'kanban', label: '看板', icon: KanbanSquare },
            { id: 'dag', label: '依赖图', icon: GitBranch },
          ...
          {activeView === 'dag' && (
            <div className="h-full p-5">
              <DAGVisualization
                ...
                onAddDependency={async (fromTaskId, toTaskId) => {
                  try {
                    await api.post(`/command-center/tasks/${toTaskId}/dependencies`, {
                    ...
                    setError(err.message || '添加依赖失败');
                  }
                }}
                onRemoveDependency={async (taskId, dependsOnTaskId) => {
                  try {
                    await api.delete(`/command-center/tasks/${taskId}/dependencies/${dependsOnTaskId}`);
                    await loadDAG();
                    setNotice('依赖关系已移除');
                  } catch (err: any) {
                    setError(err.message || '移除依赖失败');
                  }
                }}
              />
            </div>
...
{JSON.stringify(details, null, 2)}
          ...
          <section className="rounded-3xl border border-gray-800 bg-dark-900/90 p-4">
            <ApprovalGateManager
              ...
              availableSkills={actorSkillAssignments.map(s => s.skillName)}
            />
          </section>
